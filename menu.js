...
exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         const id = m.chat
         const db_type = process?.env?.DATABASE_URL ? /mongo/.test(process.env.DATABASE_URL)
            ? 'Mongo'
         ...
         if (style === 1) {
            ...
            if (text) {
               ...
               client.replyButton(m.chat, [{
                  ...
                  body: 'Automatización de WhatsApp',
                  thumbnail: Func.isUrl(setting.cover) ? setting.cover : Buffer.from(setting.cover, 'base64')
               })
            }
         }
