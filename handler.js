...
module.exports = async (client, ctx) => {
   ...
   try {
      ...
      if (m.isGroup && !groupSet.stay && (new Date * 1) >= groupSet.expired && groupSet.expired != 0) {
         return client.reply(m.chat, Func.texted('italic', '🚩 El tiempo del bot ha expirado y saldrá de este grupo, gracias.', null, {
      ...
      if (users && (new Date * 1) >= users.expired && users.expired != 0) {
         return client.reply(users.jid, Func.texted('italic', '🚩 Tu paquete premium ha expirado, gracias por comprar y usar nuestro servicio.')).then(async () => {
            users.premium = false
            users.expired = 0
      ...
      if (m.isGroup && !m.isBot && users && users.afk > -1) {
         client.reply(m.chat, `Has vuelto a estar en línea después de estar inactivo por : ${Func.texted('bold', Func.toTime(new Date - users.afk))}\n\n• ${Func.texted('bold', 'Razón')}: ${users.afkReason ? users.afkReason : '-'}`, m)
         users.afk = -1
         users.afkReason = ''
         users.afkObj = {}
      }
      ...
      if (body && !setting.self && core.prefix != setting.onlyprefix && commands.includes(core.command) && !setting.multiprefix && !env.evaluate_chars.includes(core.command)) return client.reply(m.chat, `🚩 *¡Prefijo incorrecto!*, este bot usa el prefijo : *[ ${setting.onlyprefix} ]*\n\n➠ ${setting.onlyprefix + core.command} ${text || ''}`, m)
      ...
      if (prefix && !commands.includes(command) && matcher.length > 0 && !setting.self) {
         if (!m.isGroup || (m.isGroup && !groupSet.mute)) return client.reply(m.chat, `🚩 El comando que estás usando es incorrecto, prueba con las siguientes recomendaciones :\n\n${matcher.map(v => '➠ *' + (prefix ? prefix : '') + v.string + '* (' + v.accuracy + '%)').join('\n')}`, m)
      }
      if (body && prefix && commands.includes(command) || body && !prefix && commands.includes(command) && setting.noprefix || body && !prefix && commands.includes(command) && env.evaluate_chars.includes(command)) {
         if (setting.error.includes(command)) return client.reply(m.chat, Func.texted('bold', `🚩 El comando _${(prefix ? prefix : '') + command}_ está desactivado.`), m)
         ...
         for (let name in is_commands) {
            ...
            if (!m.isGroup && !['owner', 'menfess', 'scan', 'verify', 'payment', 'premium'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) {
               client.sendMessageModify(m.chat, `⚠️ Usar el bot en chat privado es solo para usuarios premium. ¿Quieres actualizar al plan premium? Envía *${prefixes[0]}premium* para ver los beneficios y precios.`, m, {
                  largeThumb: true,
            ...
            if (cmd.restrict && !isPrem && !isOwner && text && new RegExp('\\b' + setting.toxic.join('\\b|\\b') + '\\b').test(text.toLowerCase())) {
               client.reply(m.chat, `⚠️ Has violado los *Términos y Condiciones* de uso del bot al utilizar palabras clave prohibidas. Como penalización por tu infracción, serás bloqueado y expulsado.`, m).then(() => {
                  users.banned = true
            ...
            if (cmd.limit && users.limit < 1) {
               client.reply(m.chat, `⚠️ Has alcanzado el límite y se restablecerá a las 00:00\n\nPara obtener más límite, actualiza a un plan premium.`, m).then(() => users.premium = false)
               continue
            }
            if (cmd.limit && users.limit > 0) {
               ...
               if (users.limit >= limit) {
                  ...
                  client.reply(m.chat, Func.texted('bold', `⚠️ Tu límite no es suficiente para usar esta función.`), m)
                  continue
               }
            }
         ...
         for (let name in is_events) {
            ...
            if (event.limit && !event.game && users.limit < 1 && body && Func.generateLink(body) && Func.generateLink(body).some(v => Func.socmed(v))) return client.reply(m.chat, `⚠️ Has alcanzado el límite y se restablecerá a las 00:00\n\nPara obtener más límite, actualiza al plan premium.`, m).then(() => {
               users.premium = false
               users.expired = 0
            })
